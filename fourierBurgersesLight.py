...
def solve(X0=X00, T0=T00, dx=dx0, dt=dt0, u_init=u_init0, eps=eps0, eta=eta0, flux=2, scheme=6, ordr=2, plots=1,vb=0):
	...
	if scheme==1:		# This is the scheme used in expdelta
		for k in range(nt-1):
			if k%1000 == 0: print("step " + str(k) + " of " + str(nt))
			...
			J = getJ(u,ux)
			u = fsmooth(u - dt*Dx(J,dx))	# Update u (with smoothing from artificial viscosity
		...
		for k in range(nt-1):
			...
			u = fsmooth(u - dt*(Dx1(J,dx,1)))		# Update u
	...
	if plots==1:
		pylab.figure()
		pylab.plot(x,u); pylab.show()
	
	return u,J,(dx,dt,X0,T0)
