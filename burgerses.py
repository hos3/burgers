...
def solve(nx=nx0, nt=nt0, dx=None, dt=dt0, u_init=u_init0, eps=eps0, eta=eta0, flux=0, scheme=1, plots=1):
	...
	if scheme==1:		# This is the scheme used in expdelta
		for k in range(nt-1):
			...
			J = getJ(u[:,k],ux)
			u[:,k+1] = u[:,k] - dt*Dx(J,dx) + dt*eta*(up+um-u[:,k]*2.)/(dx**2)		# Artificial viscosity
		...
		for k in range(nt-1):
			...
			u[:,k+1] = u[:,k] - dt*(Dx1(J,dx,1)) + dt*eta*(up+um-u[:,k]*2.)/(dx**2)		# Update u
	...
	# The idea of each of the following is identical to scheme 2, so I'll forgo excessive commenting. 
		...
		for k in range(nt-1):
			...
			u[:,k+1] = u[:,k] - dt*(Dx1(J,dx,1)) + dt*eta*(up+um-u[:,k]*2.)/(dx**2)
